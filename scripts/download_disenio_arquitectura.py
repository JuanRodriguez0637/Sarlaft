#!/usr/bin/env python3
"""
download_disenio_arquitectura.py
Descarga Sarlaft 4.0 → Documentación Técnica → Diseño - Arquitectura
usando la misma estructura estándar que download_motor_evaluacion.py:

    docs/Diseño - Arquitectura/
    ...
    │   ├── img/
    │   ├── attachments/
    │   └── <Pagina>.md           (todos los descendientes)
    └── ...

Espacio  : EPA
Raíz     : Diseño - Arquitectura  (ID: 1804697669)
Output   : D:\\Sarlaft 4.0\\docs\\Diseño - Arquitectura\\
"""
...
# ─── Rutas ────────────────────────────────────────────────────────────────────
ARQ_ROOT  = Path(DOCS_ROOT) / "Diseño - Arquitectura"
...
SECTIONS = [
    ("1804927175", "Requisitos No Funcionales"),
    ("1804861588", "Diagramas de Arquitectura"),
    ("1801126348", "Contextualización de Arquitectura"),
    ("1801159177", "Documento Diseño Técnico"),
    ("1856537007", "Estándar nombramiento Base de Datos"),
    ("1866629127", "Acceso a la Cache"),
    ("3214016543", "Diseño Funcionalidades"),
    ("4703420517", "Información de secretos"),
]


# ─── Helpers ──────────────────────────────────────────────────────────────────
def to_pascal(title: str) -> str:
    """'9. Consulta información de Cliente (getClient)' → '9ConsultaInformacionDeClienteGetClient'"""
...
def get_all_descendants(page_id: str, depth: int = 0) -> list:
    """Retorna [(id, title, depth)] para todos los descendientes en orden de árbol."""
    items = []
...
def process_attachments(page_id: str, section_dir: Path) -> dict:
    """Descarga adjuntos referenciados y retorna {filename: rel_md_path}."""
...
def process_section(section_id: str, section_title: str) -> list:
    ...
    print(f"  [OK] {len(created)} archivo(s) en {section_dir}")
    return created


# ─── Raíz ─────────────────────────────────────────────────────────────────────
def process_root() -> None:
    """Genera index.md en la raíz de arquitectura con la página padre."""
    print(f"\n{'='*60}")
    print(f"RAÍZ: Diseño - Arquitectura  [{ROOT_ID}]")
    print("=" * 60)

    root_dir = ARQ_ROOT
    (root_dir / "img").mkdir(parents=True, exist_ok=True)
    (root_dir / "attachments").mkdir(parents=True, exist_ok=True)

    page = get_page(ROOT_ID)
    md   = build_page_md(page, root_dir, "Diseño - Arquitectura", "./index.md")

    md += "\n\n## Secciones\n\n| Sección | Enlace |\n|---------|--------|\n"
    for _, stitle in SECTIONS:
        folder = safe_folder(stitle)
        md += f"| {stitle} | [./{folder}/index.md](./{folder}/index.md) |\n"

    (root_dir / "index.md").write_text(md, encoding="utf-8")
    print(f"  [OK] index.md raíz generado")
...
def main() -> None:
    t0 = datetime.now()
    print("=" * 60)
    print("Diseño - Arquitectura — Descarga Confluence")
    print(f"Output: {ARQ_ROOT}")
    print("=" * 60)
    ...
    idx = (
        "# Diseño - Arquitectura — Índice\n\n"
        f"> **Fuente Confluence:** [Diseño - Arquitectura]"
        f"({BASE_URL}/wiki/spaces/EPA/pages/{ROOT_ID})  \n"
    ...
    (ARQ_ROOT / "_INDICE.md").write_text(idx, encoding="utf-8")

    elapsed = int((datetime.now() - t0).total_seconds())
    print(f"\n{'='*60}")
    print(f"COMPLETADO en {elapsed}s")
    total_pages = sum(len(p) for _, p, _ in summary)
    print(f"Páginas .md generadas: {total_pages + 1} (raíz) en {len(SECTIONS)} secciones")
    print(f"Índice: {ARQ_ROOT / '_INDICE.md'}")
    print("=" * 60)
